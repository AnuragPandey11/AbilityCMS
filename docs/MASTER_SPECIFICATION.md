...
**Version:** 3.5 · **Date:** 25 September 2026 · **Status:** Living document
...
| **Collector** | The enclosure a set of Devices sits in — an MCR, an ICR, a panel — named by one segment of the topic. **Never a Device**: it publishes nothing, carries no current, and appears in the Single Line Diagram as a box drawn *around* its Devices, never as a node in the chain. Optional: a Device whose topic has no Collector segment sits in none, which is a real answer. See §3.4. | `devices.collector_code` | `collector_code` | "Collector" | logger, gateway, RTU, room, panel |
...
| `collector_code` | **What enclosure is it in?** | Communication | The box drawn around it in the diagram; collector-failure correlation |
...
**`collector_code` is not a fourth grouping.** It is the communication grouping's *name*, sitting beside `reports_via_device_id` rather than replacing it, because the two say different things: "relayed by that datalogger" is a Device-to-Device relationship, "in the same room as that Inverter" is not. A Device may have either, both, or neither, and the health sweep correlates on whichever is present — grouping by the transmitting Device where one is recorded, since it is the more specific claim.

**Status: CONFIRMED (client, 19 Sep 2026) · IN SCHEMA (migration 0022).** The client's statement is direct: *a Collector is a logical box that holds things, and is never a Device.* Commissioning previously registered the `{collector_code}` topic segment as an `MCR_SECTION` Device that everything beneath it pointed at, which put a *room* in the electrical diagram between the Inverters and the meter. It no longer does. A Collector-Device an operator registered deliberately is untouched; `python -m solarcms.cli collectors-from-topics` retires the ones commissioning created, dry-run by default.
...
**Which Alarm Rule applies to a Device.** AGREED (development team, 25 Sep 2026) / BUILT. A rule has an **owner** — the platform (`client_id` NULL, inherited by every Client) or one Client (reaches that Client's Devices only) — and a **scope**: `device`, `plant`, `device_type`, `client` or `global`. Among the rules that reach a Device, one wins per rule code: **the narrowest scope wins** (device → plant → device_type → client → global), and **at the same scope a Client's own rule beats the platform default**. Ownership never outranks scope, so a Client rule wider than the default it meant to replace does not replace it. Rules with different codes all apply. The same precedence governs the per-Reading rules and the absence rules (`COMM_LOST` and its siblings). An open Alarm belongs to the **code**, not to the rule that raised it: when a Client's rule takes over from the default, an Alarm already open stays the one Alarm for that fault and closes when the fault clears (v3.5).
...
| Version | Date | Change |
|---|---|---|
| **3.5** | 25 Sep 2026 | **No threshold or Digital Input Alarm had ever been raised.** Found while testing v3.4, BUILT; no schema change. The alarm worker notified subscribers in the same transaction as the raise, under a database role with no access to `notification_subscriptions`, so every raise failed and rolled back: the table held 370 absence Alarms and none from any threshold or contact rule — no over-temperature, no VCB trip, no Buchholz trip. Alarm evaluation was therefore not in service for per-Reading rules at any point before this date; any report or availability figure that relied on those Alarms had nothing to rely on. The worker now writes under the scheduler role, as the health sweep already did. Two further defects fixed in the same place: a worker restart forgot every open Alarm, leaving it active after its fault had cleared; and a Client rule replacing a platform default raised a second Alarm for the same fault while the first never closed. One Alarm is now held per rule code per Device (§6.4), whichever rule with that code raised it. |
| **3.4** | 25 Sep 2026 | **Alarm Rule precedence: scope first, then the owner.** AGREED (development team) / BUILT; no schema change. Where two rules with the same code reach a Device at the **same scope**, the Client's own rule now beats the platform default (§6.4). The tie previously went to whichever row the database returned first — the worker's query had no `ORDER BY` — and because `cli seed` rewrites every platform default, re-seeding could silently reverse which threshold was in force. It also means a Client replaces a Type-scoped default for all of its Plants, including future ones, with one rule at that Type, where before only a rule per Plant was reliable. Scope still decides first, so a Client rule wider than the default does not replace it; the Alarm Rules screen says so on the rule. The health sweep's absence rules now follow the same precedence (they previously read the platform row only, so a Client's `COMM_LOST` was stored and never consulted). `POST /alarm-rules` takes the owner explicitly from a platform administrator — a Client, or null for a platform default — where it used to follow the session, so a Super Admin with no Client selected created a rule for every Client without being asked; a rule scoped to a Plant, Device or Client of a different owner, or to one that does not exist, is refused because it could never apply. `GET /alarm-rules` shows a platform administrator every Client's rules, with the owner and the scope's target named. |
| **3.3** | 24 Sep 2026 | **Expected power, as a ceiling; power against radiation on locked axes; one Inverter in full.** ⚠ PROPOSED (the client has supplied no expected-power basis) / BUILT, frontend only, no schema change. *Exp Power (DC)* = plane-of-array irradiance × DC nameplate ÷ 1,000 W/m² (IEC 61724 reference power, the PR denominator); *Exp Power (AC)* = the DC reference × the Inverters' measured efficiency (median while generating), limited to the AC nameplate — below DC by the conversion loss, as on a real Plant. Lossless and not temperature-corrected — ceilings, not forecasts; replaced when the client names their basis. The single-Plant power trend overlays WMS `DIRECT_RADIATION` (new slot `env.direct_radiation`) on a second y-axis **derived from the first through the DC nameplate**, the one exception to Guardrail 22 and only with a DC capacity recorded. `GET /devices/{id}/operating-status` applies the v3.2 start/stop rule to one Device; the Inverter view shows it beside the Inverter's `DEVICE_STATUS` code, untranslated, since no meaning has been supplied for its values. |
| **3.2** | 24 Sep 2026 | **Plant start and stop are judged on the Inverters, with hysteresis, and derived from history.** ⚠ PROPOSED by the project — it **supersedes the client's supplied rule** (`TAG_CATALOGUE.md` §2.15.4: one 0.1 MW threshold on "the active power") and wants the client's confirmation — / BUILT. The Plant starts at the first minute its Inverters' summed `AC_ACTIVE_POWER` rises above **0.5 kW** and stops at the minute it falls back to **0 kW**; a restart the same day takes the stop back, so the day's last fall stands (the T-17 reading). Constants `PLANT_OPERATING_SOURCE`, `PLANT_START_ABOVE_KW`, `PLANT_STOP_AT_OR_BELOW_KW` replace `PLANT_RUNNING_THRESHOLD_KW` in `assumptions.py`; the rule is `domain/operating.py`. `GET /plants/{id}/operating-status` folds it over one-minute history on every request (a restart forgets nothing; yesterday needs no copy), marks a transition that happened inside a silence as unobserved rather than printing it as exact, and reports `unknown` — never `running` — for Inverters that went quiet while generating. The scheduler's `PLANT_START_TIME`/`PLANT_STOP_TIME` now use the same rule, which also ends a defect: the stop time was rewritten on every tick after sunset and read ~23:54 by the rollover. Peak load is the maximum of whatever the Current Power slot resolves to. **Grid status** is read from the VCB's `VCB_ON_FEEDBACK` contact — ⚠ ASSUMED, an interpretation; a Plant with no VCB has no grid status, never an assumed "connected". Slot `env.cloud_cover` added (WMS `CLOUD_COVER`); `/kpis` returns `specific_yield`. No migration. |
| **3.1** | 19 Sep 2026 | **Onboarding reads the broker instead of asking.** BUILT; no schema change. Engineers publish before we onboard, so Client codes, Plant codes, Collectors, Device codes and payload keys are observable facts before any form is filled in — a blank form was asking people to reproduce them from memory, and one wrong character produced a Device that looked correctly registered and silently decoded nothing. New `/discovery/{clients,plants,topic}` turn registration into confirmation: the topic supplies the code, the Collector is read from it, `expected_interval_s` is the **measured** gap between messages rather than the assumed 60 s, and the Tag mapping is the payload keys actually arriving matched against the catalogue with anything unmatched shown rather than hidden. ⚠ Reads `mqtt_raw_v`, **never MQTT** — Guardrail 3 forbids ingestion in the API process — so it reflects what ingest has *received*, and is empty when ingest is down. ⚠ **Super Admin only**: an unregistered topic is quarantined with `client_id = NULL` because attributing it by reading the topic is the inference Guardrail 5 forbids at ingest, so `mqtt_raw_v` shows those rows to a platform administrator alone. Discovery only proposes; registration stays deliberate. **`POST /clients` now creates the Client and its first User in one transaction** — a Client nobody can sign into looks finished on every screen — defaulting to `admin`, because `app_can_see_plant` grants an admin every Plant of their own Client and any other role would start with zero Plants (Guardrail 7). **`DELETE /devices/{id}` added**, refusing a Device with stored Readings unless forced: `readings` carries no foreign key, so a delete would strand its history under an id that resolves to nothing. **Planned Device counts are no longer an input** (OPEN-21 superseded in practice): a figure typed before any Device existed drifted from the moment one was registered, and the count that matters — `count(*)` over registered Devices by Type — is derived and shown when editing. The table and API field remain unused rather than dropped. |
| **3.0** | 19 Sep 2026 | **A Collector owns one outward edge, the boundary becomes a rule, and the diagram draws Devices that carry no current.** CONFIRMED (client, 19 Sep 2026) / BUILT in migration 0024. §1.1 and §3.4 stand: a Collector is an enclosure and **still never a Device** — no Model, no Type, no Tags, no topic, no Readings, and `devices` gains no foreign key to it. What it gains is a row (`plant_collectors`: plant, code, `parent_device_id`, note) so that *what the room feeds into* is stated once, on the box, instead of being repeated on all seventeen Inverters inside it and left for the reader to notice is identical. **Membership is unchanged and stays `devices.collector_code`**, because it comes from the topic and the topic is the sole authority for origin (Guardrail 5); a Collector needs no row to exist, so the table is expected to be sparse and every join to it is LEFT. With the box owning an edge, **a Device inside a Collector may no longer be wired to one outside it** — `domain/sld.crosses_collector_boundary`, enforced in the API rather than by a constraint because a CHECK cannot see the parent row and a composite foreign key is satisfied vacuously whenever either `collector_code` is NULL (MATCH SIMPLE). Hierarchy *within* a Collector is explicitly still allowed. Kular Green's existing wiring was migrated: 17 per-Inverter parents pointing at the MFM became one `MCR → MFM` edge. **Separately, `excluded_not_in_power_path` is now rendered** — dashed, labelled "no current", placed inside its Collector's box — so a Weather Station in the MCR is visible as being in the MCR. ⚠ `build_sld` is **unchanged** and the electrical tree still contains power-path Devices only: the model excludes them, the picture shows them, and conflating those two is what §2.3 warns against. Two new Guardrails (13 in BACKEND_SPEC §14): never wire across a Collector wall, and never draw a wire from a Collector to its own members. |
| **2.9** | 19 Sep 2026 | **Every dashboard figure was hours stale, for two unrelated reasons, neither of which raised anything.** Found by inspection after the operator reported delayed numbers; BUILT, behaviour only, no stored figure changed and no formula touched (OPEN-14/15/16 unaffected). **(1) Real-time aggregation had never been on.** `create_continuous_aggregate` takes `real_time=True` and 0005 always used it, but the helper only ever *wrote* the flag in the `False` branch and let the `True` case fall to TimescaleDB's default — which flipped in 2.13 from `materialized_only = false` to `true`. On 2.30 every tier was therefore created the opposite of what was asked for. Because the cascade is hierarchical, the lag compounded (`agg_1m` ~2 min → `agg_15m` ~32 min → `agg_1h` ~2.5 h → `agg_1d` ~4.5 h); measured at 20:09 UTC, `agg_1h`'s newest bucket was 17:00. Migration 0023 sets `materialized_only = false` on all four and the helper now writes the flag in both directions, since intent expressed by omission cannot survive a default changing underneath it. Cost measured at 5–10 ms per day-range query. **(2) `GET /plants/{id}/kpis` hardcoded the hourly tier** for every period, making *today* — the figure watched minute to minute — the worst-served on the platform. It now calls `domain/tiering.select_tier`, the same function the readings read path uses, which returns `agg_1m` for a day range; the tier is reported back as `source_tier` so "why does this number look old" is answerable from the response. **(3) On the frontend, the per-Plant KPI fan-out existed twice** — in `usePlantFleet` and again in `PortfolioDashboard` — both with `staleTime` and neither with `refetchInterval`, which is not a slow refresh but no refresh: every Portfolio, Plant Overview and Plant List figure was fixed at page load. Unified into one `usePlantKpiFanout` on a 10 s cadence, with `refetchOnWindowFocus` enabled for live figures only (a throttled background tab cannot be trusted to keep its own timer). `/plants/{id}/dashboard`, which already resolved against the Redis values ingest writes per message, moved from 30 s to 5 s. Measured after: Portfolio and Plant List re-ask every ~11 s, Single Plant's slots every ~6 s, against zero requests before. |
| **2.8** | 19 Sep 2026 | **A Collector is an enclosure, not a Device**, and the topic may omit it. CONFIRMED by the client 19 Sep 2026; IN SCHEMA in migration 0022. `devices.collector_code` records the `{collector_code}` segment as a *name*; commissioning-from-broker no longer registers a Device for it, and `python -m solarcms.cli collectors-from-topics --apply --retire-devices` repairs Plants commissioned under the old behaviour (re-pointing children at the retired Device's own parent first, so the chain closes over the gap). The Single Line Diagram, the plant schematic and the hierarchy editor draw a Collector as a **dashed outline around** the Devices sharing its name, never as a node — an outline may never enclose a Device that is not in it, so `build_sld` keeps same-Collector siblings adjacent and the renderer splits a group into several outlines rather than swallowing a stranger. `correlate_collector_failures` now groups on either form (`CollectorKey = int | str`), which is what keeps a failed MCR one Alarm rather than twenty, now that the MCR is not a Device. **Two new `topic_patterns` rows** for the five-segment shape `scms/v1/{client_code}/{plant_code}/{device_code}` (and its uppercase twin): equipment that sits in no enclosure publishes directly under the Plant, and `collector_code` NULL is the correct record of that, not a gap. **A Plant is filed under a Client before it exists:** `POST /plants` accepts `client_id`, **required** for a Super Admin (who is a member of no Client, and whose Plants previously landed under whichever Client the session had been switched into) and **ignored** for a Client Admin (I-9 unchanged — their Client comes from the session and nothing else). `GET /plants` returns `client_id`/`client_code`/`client_name` per row and takes an optional `client_id` filter, which ANDs with the policy and can therefore never widen what a Client Admin sees. The onboarding wizard makes the Client an explicit first step rather than a session switch. No units, formulas or thresholds are introduced; OPEN-14, OPEN-15 and OPEN-16 are unaffected. |
